// Talking to the classroom server (server/classroom.js): JSON requests, live
// updates over Server-Sent Events, and remembering which class this tab is in
// so a reload or a Chromebook that went to sleep can rejoin. Kept per tab
// (sessionStorage), so several tabs can be different students when testing. Loaded as a plain script
// (window.BlockoutClassroom); the screens themselves live in game.js.
(function (root) {
  'use strict';
  ...
  let unloading = false;
  root.addEventListener('pagehide', () => (unloading = true));
  root.addEventListener('beforeunload', () => (unloading = true));
